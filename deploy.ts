...
async function deployStack(): Promise<void> {
  ...
  const environment: string = readlineSync.question(
    "Enter the environment (dev, staging, prod): "
  );

  // Get the database username and password from the user
  const dbUsername: string = readlineSync.question(
    "Enter the database username: "
  );
  const dbPassword: string = readlineSync.question(
  ...
  deploySAM(environment, secretName, environment);
}
...
function deploySAM(
  ...
  const command = `sam deploy --stack-name ${stackName} --resolve-s3 --s3-prefix ${stackName} --capabilities CAPABILITY_IAM --parameter-overrides DBSecretKey=${secretName} StackEnvironment=${environment} --no-confirm-changeset`;
